...
function activate(context) {
  ...
  let downloadRemote = vscode.commands.registerCommand('anes.downloadRom', function (remoteConfig) {
    ...
    console.log('start download : ',JSON.stringify(remoteConfig))
    let remoteMetaFile = fs.readFileSync(path.join(userRoot, '.anes', 'remote','meta.json'));
    let remoteMetaString = remoteMetaFile.toString();
    let remoteMeta = JSON.parse(remoteMetaString);
    let newTask = {
      name: remoteConfig.label,
      downloadStatus: 1
    }
    let findIndex = remoteMeta.findIndex((v,i,o)=>v.name==newTask.name)
    let downloadStatusEnum = ["not start","downloading...","downloaded"]
    if (findIndex>-1) {
      let task = remoteMeta[findIndex]
      if (task.downloadStatus==2) {
        vscode.window.showInformationMessage(`download status :${downloadStatusEnum[task.downloadStatus]}`);
        return
      }else{
        remoteMeta[findIndex] = Object.assign(task,{downloadStatus:1})
      }
    }else{
      remoteMeta.push(newTask)
          ...
          if (!err){
            ...
            if(remoteConfig.fileName.indexOf(".zip")>-1){
              ...
              compressing.zip.uncompress(zipPath, localPath).then(function(){
                ...
                vscode.window.showInformationMessage(`unzip error :${err}`);
                reject(err);
              });
            }else{
            ...
            remoteMeta.forEach((meta)=>{
              if(meta.name != remoteConfig.label && !newMeta.find(v=>v.name==meta.name)){
                newMeta.push(meta);
              }
            })
            vscode.window.showInformationMessage(`Game: ${remoteConfig.label} download err, and err info is ${err}!`);
            fs.writeFileSync(path.join(userRoot, '.anes', 'remote','meta.json'),JSON.stringify(newMeta));
            reject(err)
          }
        },progress);
      });
